...
class AffineGridGenerator(Function):
    ...
    def _enforce_cudnn(input):
        if not cudnn.enabled:
            raise RuntimeError("AffineGridGenerator needs CuDNN fo processing CUDA inputs, but CuDNN is not enabled")
...
if __name__=='__main__':
    theta = torch.rand([8, 2, 3])
    img = torch.rand([8, 3, 14, 14])
    #gen_grid = AffineGridGenerator()
    #gen_grid.forward()
    grid = affine_grid_generator(theta, img.shape)
    print(grid)